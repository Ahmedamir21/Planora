...
async function main() {
  ...
  const feedback = await feedbackCall(message);
  ...
  assert(collected.items[0].message === message.message && collected.summary.newCount === 1 && collected.summary.averageRating === 8 && collected.summary.categories['Schedule generator'] === 1, 'Feedback summary wrong');
  ...
  assert((await feedbackCall({ ...message, message: '' })).code === 201, 'Rating-only feedback rejected');
