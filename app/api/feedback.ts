...
export const FEEDBACK_CATEGORIES = ['Ease of use', 'Schedule generator', 'AI assistant', 'Design & mobile', 'Feature idea', 'Other'] as const;
...
export type Feedback = {
  ...
  message: string; status: 'new' | 'reviewed' | 'archived'; note?: string; reviewedBy?: string; reviewedAt?: string;
};
...
export default async function handler(req: any, res: any) {
  ...
  if (!FEEDBACK_CATEGORIES.includes(body.category) || typeof body.message !== 'string' || body.message.length > 1200 || !Number.isInteger(body.rating) || body.rating < 1 || body.rating > 10) return res.status(400).json({ error: 'Choose a rating from 1 to 10. Comments can be left blank.' });
  const item: Feedback = { id: randomUUID(), at: new Date().toISOString(), category: body.category, rating: body.rating, message: body.message.trim(), status: 'new' };
  try {
